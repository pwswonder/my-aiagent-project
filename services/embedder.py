...
# === 안전 호출 유틸 (invoke 우선) ============================================
def _safe_get_docs(retriever, query: str, k: int):
    """retriever.invoke() 우선, 실패 시 get_relevant_documents() 폴백"""
    docs = []
    try:
        if hasattr(retriever, "invoke"):
            docs = retriever.invoke(query) or []
        else:
            raise AttributeError("invoke not available")
    except Exception:
...
class FailSafeRetriever(BaseRetriever):
    """
    1순위: base(압축/기본)
    2순위: ensemble (Dense+BM25, 넓게)
    3순위: BM25 only
    4순위: Dense only
    ▶ 각 단계에서 hit 수가 min_hits 이상이면 즉시 반환.
    """

    base: Any  # ContextualCompressionRetriever 또는 base_ret
    ensemble_top: Any  # EnsembleRetriever (fallback용, k 크게)
    bm25_top: Any  # BM25 only retriever (k 크게)
    dense_top: Any  # Dense only retriever (k 크게)
    top_k: int = 5
    min_hits: int = 1
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        hits = _safe_get_docs(self.base, query, self.top_k)
        if len(hits) >= self.min_hits:
            _dbg(f"fs/base hits={len(hits)}")
            return hits[: self.top_k]

        cand = []
        # Ensemble fallback
        e = _safe_get_docs(self.ensemble_top, query, max(self.top_k, 8))
        cand = _dedup_docs(hits + e)
        if len(cand) >= self.min_hits:
            _dbg(f"fs/ensemble hits={len(cand)}")
            return cand[: self.top_k]

        # BM25 only
        b = _safe_get_docs(self.bm25_top, query, max(self.top_k, 8))
        cand = _dedup_docs(cand + b)
        if len(cand) >= self.min_hits:
            _dbg(f"fs/bm25 hits={len(cand)}")
            return cand[: self.top_k]

        # Dense only
        d = _safe_get_docs(self.dense_top, query, max(self.top_k, 8))
        cand = _dedup_docs(cand + d)
        _dbg(f"fs/dense hits={len(cand)}")
        return cand[: self.top_k]
...
def embedder(state: EmbedState) -> EmbedState:
    ...
    # ---- (유지) 경량 압축 ----
    # 기존에는 여기서 retriever = ... 로 끝났지만,
    # 이제는 base_ret에 "압축 여부"만 반영하고, 반환 직전 Fail-Safe 래퍼로 감싼다.
    use_compress = os.getenv("RAG_COMPRESS", "1") != "0"
    if use_compress:
        # 임계치 완화는 .env 로 제어 : RAG_SIM_THRESHOLD=0.72 권장
        sim_th = _get_env_float("RAG_SIM_THRESHOLD", 0.76)
        compressor = EmbeddingsFilter(
            embeddings=embedding_model, similarity_threshold=sim_th
        )
        base_ret = ContextualCompressionRetriever(
            base_compressor=compressor, base_retriever=base_ret
        )

    # === Fail-Safe 래퍼 계층 추가 ==========================================
    # 압축/앙상블에서 0건이 나와도 최소 min_hits 건은 반드시 반환하도록 폴백 파이프 구성

    # (1) 폴백용 k 값을 넉넉히 확보
    fallback_k = max(fetch_k, top_k * 6)

    # (2) Dense only (넓은 후보)
    dense_top = vectorstore.as_retriever(
        search_type="similarity", search_kwargs={"k": fallback_k}
    )

    # (3) BM25 only (넓은 후보) — 버전 차이 대응을 위해 preprocess 인자 2종 시도
    try:
        bm25_top = BM25Retriever.from_texts(
            [t if isinstance(t, str) else "" for t in texts],
            metadatas=metadatas,
            preprocess_func=_bm25_preprocess,  # langchain>=0.1.x
        )
        ...
        bm25_top = BM25Retriever.from_texts(
            [t if isinstance(t, str) else "" for t in texts],
            metadatas=metadatas,
            preprocess_fn=_bm25_preprocess,  # older versions
        )
    bm25_top.k = fallback_k

    # (4) Ensemble (Dense+BM25, 넓게)
    ensemble_top = EnsembleRetriever(
        retrievers=[dense_top, bm25_top],
        weights=[
            w_dense,
            w_sparse,
        ],  # 위에서 파싱한 RAG_HYBRID_WEIGHTS 사용(기본 0.6,0.4)
    )

    # (5) 최소 히트 수 (기본 1) — .env: RAG_MIN_HITS=1
    min_hits = _get_env_int("RAG_MIN_HITS", 1)

    # (6) Fail-Safe 래퍼로 최종 retriever 구성
    #  - base_ret : (압축이 켜졌다면) ContextualCompressionRetriever, 아니면 TopKEnsembleRetriever/MMR
    #  - base -> ensemble_top -> bm25_top -> dense_top 순으로 폴백
    retriever = FailSafeRetriever(
        base=base_ret,
