...
# st.set_page_config(page_title="문서 히스토리", layout="wide")
# st.title("📄 업로드된 문서 히스토리")
...
# except Exception as e:
#     st.error(f"문서 목록을 불러오는 데 실패했습니다: {e}")
#     documents = []
...
#             st.markdown(f"**🧠 기술 도메인:** `{doc.get('domain', 'N/A')}`")
#             st.markdown("**📝 문서 요약:**")
#             st.write(doc.get("summary", "요약 정보 없음"))

#             st.markdown("---")
#             st.markdown("**💬 질문 / 응답 히스토리**")

#             try:
#                 qa_res = requests.get(f"{FASTAPI_URL}/qa/{doc['id']}")
#                 qa_res.raise_for_status()
#                 qa_list = qa_res.json()
