...
# Training kwargs (optional). External trainer can consume this via: model.fit(..., **FIT_KWARGS)
FIT_KWARGS = {}
# {{CUSTOM_BLOCK:FIT_KWARGS}}
...
num_classes = 1000
stem_filters = 64
stage_blocks = [3, 4, 6, 3]  # per stage
stage_filters = [64, 128, 256, 512]
bottleneck = True  # True: bottleneck(1x1-3x3-1x1), False: basic(3x3-3x3)
use_se = False  # Squeeze-and-Excitation on/off
se_reduction = 16
stochastic_depth = 0.0  # 0.0 ~ 0.5 권장
dropout_rate = 0.1
...
class StochasticDepth(layers.Layer):
    # {{CUSTOM_BLOCK:model_head}}    """Per-sample stochastic depth (a.k.a. DropPath)."""
...
class SEBlock(layers.Layer):
    ...
    def __init__(self, channels: int, reduction: int = 16, **kwargs):
        ...
        self.fc1 = layers.Dense(
            max(1, self.channels // self.reduction), activation="relu"
        )
...
class BasicBlock(layers.Layer):
    ...
    def __init__(
        self, filters, stride=1, use_se=False, se_reduction=16, drop_prob=0.0, **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.conv1 = layers.Conv2D(
            self.filters, 3, strides=self.stride, padding="same", use_bias=False
        )
    ...
    def build(self, input_shape):
        ...
        if (self.stride != 1) or (in_ch != out_ch):
            self.proj = keras.Sequential(
                [
                    layers.Conv2D(
                        out_ch, 1, strides=self.stride, padding="same", use_bias=False
                    ),
...
class BottleneckBlock(layers.Layer):
    ...
    def __init__(
        ...
        self.bn1 = layers.BatchNormalization()
        self.conv2 = layers.Conv2D(
            width, 3, strides=self.stride, padding="same", use_bias=False
        )
        self.bn2 = layers.BatchNormalization()
        self.conv3 = layers.Conv2D(
            self.filters * self.expansion, 1, strides=1, padding="same", use_bias=False
        )
...
def make_stage(
    x, filters, blocks, block_cls, base_prob, end_prob, start_idx, name_prefix
):
    """
...
def build_model():
    ...
    # Stem
    x = layers.Conv2D(
        stem_filters, 7, strides=2, padding="same", use_bias=False, name="stem_conv"
    )(inputs)
    ...
    if int(num_classes) == 1:
        ...
        outputs = layers.Dense(int(num_classes), activation="softmax", name="cls_head")(
            x
        )
    ...
    # {{CUSTOM_BLOCK:compile_override}}    model.compile(optimizer=opt, loss=loss_fn, metrics=metrics)
    # {{CUSTOM_BLOCK:model_body_extra}}    return model


# {{CUSTOM_BLOCK:callbacks}}
